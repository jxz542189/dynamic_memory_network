...
from model.dmn.encoder import Encoder
from model.dmn.episode import Episode


class Graph(object):
    def __init__(self, mode, max_input_mask_length, vocab, dtype=tf.float32):
        self.mode = mode
        self.dtype = dtype
        self.max_input_mask = max_input_mask_length
        self.vocab = vocab
    ...
    def build(self,
        ...
        facts, question = self._build_input_module(embedding_input, input_mask,
                                                   embedding_question)
    ...
    def _build_input_module(self, embedding_input, input_mask, embedding_question):
        encoder = Encoder(encoder_type=Config.model.encoder_type,
        ...
        with tf.variable_scope("input-module") as scope:
            input_encoder_outputs, _ = encoder.build(embedding_input, input_length, scope="encoder")
            ...
            with tf.variable_scope("facts") as scope:
                ...
                facts = tf.unstack(tf.transpose(facts_stacked, [1, 0, 2]), num=self.max_input_mask)
    ...
    def _build_answer_decoder(self, last_memory):
        with tf.variable_scope("answer-module"):
            w_a = tf.get_variable("w_a",[Config.model.num_units, self.vocab],
                                  regularizer=tf.contrib.layers.l2_regularizer(reg_scale=Config.model.reg_scale))
